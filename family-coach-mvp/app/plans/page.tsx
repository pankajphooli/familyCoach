...
export default function PlansPage(){
  ...
  const [ingredientsFor, setIngredientsFor] = useState<string>(''); const [ingredients, setIngredients] = useState<string[]>([])
  const [replacingId, setReplacingId] = useState<string|null>(null); const [altOptions, setAltOptions] = useState<string[]>([])
  const [profile, setProfile] = useState<Profile|null>(null)

  // dates (rolling 7 days starting today)
  ...
  // fast hydrate from cache for mobile
  useEffect(()=>{
    try{
      const raw = localStorage.getItem(cacheKey.current)
      if(raw){
        const p = JSON.parse(raw)
        setWeekMeals(p.weekMeals||{})
        setWeekBlocks(p.weekBlocks||{})
        setTodayMeals(p.todayMeals||[])
        setTodayBlocks(p.todayBlocks||[])
      }
    }catch{}
  ...
  useEffect(()=>{ (async()=>{
    ...
    if(user){ setAuthState('in'); await initialLoad(user.id) }
    else    { setAuthState('out') }
    // keep in sync with future sign-in/out
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session)=>{
      const u = session?.user || null
      setAuthState(u ? 'in' : 'out')
      if(u) await initialLoad(u.id)
      else {
        setTodayMeals([]); setWeekMeals({}); setTodayBlocks([]); setWeekBlocks({})
      }
    })
  ...
  async function initialLoad(userId: string){
    ...
    try{
      ...
      if(profRes.error) notify('error', `Profile: ${profRes.error.message}`)
      ...
      if(!hasAny(loaded.byDateMeals) && !hasAny(loaded.byDateBlocks)){
        const ok = await ensureWeekIfNeeded(userId, (profRes.data||{}) as Profile)
        if(ok) await loadAll(userId)
      }
  ...
  function isRecipeAllowed(rec: Recipe, prof: Profile){
    ...
    if(dislikes.length && dislikes.some((d:string)=>d && name.includes(d))) return false
    const meat = normalizeName(prof.meat_policy||'')
    if(meat==='non_veg_chicken_only'){
  ...
  async function candidatesFor(tag:string, prof:Profile, limit=60): Promise<Recipe[]>{
    ...
    if(error) notify('error', `Recipes: ${error.message}`)
    const list = (data as Recipe[]) || []
    return list.filter((rec) => isRecipeAllowed(rec, prof))
  }
  async function defaultsForMeals(dayIndex:number, prof: Profile){
    try{
      ...
      return [
        { meal_type: 'breakfast', recipe_name: pickFrom(b, dayIndex,   {name:'Oat Bowl'} as Recipe).name },
        { meal_type: 'lunch',     recipe_name: pickFrom(l, dayIndex,   {name:'Chicken Wrap'} as Recipe).name },
        { meal_type: 'dinner',    recipe_name: pickFrom(d, dayIndex,   {name:'Veg Stir Fry'} as Recipe).name },
      ]
    }catch{
      const bank = [
  ...
  async function pickWorkoutFor(dayIndex:number, prof:Profile){
    ...
    if(error) notify('error', `Exercises: ${error.message}`)
    const exs = (data as Exercise[]) || []
    const allowed = exs.filter(ex => isExerciseAllowed(ex, prof))
    const a = pickFrom(allowed, dayIndex,   {name:'Glute bridge',  description:'3×12'} as Exercise)
    const b = pickFrom(allowed, dayIndex+3, {name:'Row (band)',    description:'3×12'} as Exercise)
    const c = pickFrom(allowed, dayIndex+5, {name:'Plank',         description:'3×30s'} as Exercise)
    return [
  ...
  async function ensureWeekIfNeeded(userId: string, prof: Profile){
    try{
      ...
      const havePd = new Set(((p0.data||[]) as PlanDay[]).map(r=>r.date))
      const haveWd = new Set(((w0.data||[]) as WorkoutDay[]).map(r=>r.date))
      ...
      if(insPd.length) await supabase.from('plan_days').insert(insPd)
      if(insWd.length) await supabase.from('workout_days').insert(insWd)
      ...
      for(let i=0;i<weekDates.length;i++){
        const d = weekDates[i]
        const pd = pdByDate[d]; const wd = wdByDate[d]
        if(pd && !haveMeals.has(pd)){ (await defaultsForMeals(i, prof||{})).forEach(m=>toMeals.push({...m, plan_day_id:pd})) }
        if(wd && !haveBlocks.has(wd)){ (await pickWorkoutFor(i, prof||{})).forEach(b=>toBlocks.push({...b, workout_day_id:wd})) }
      }
      if(toMeals.length)  await supabase.from('meals').insert(toMeals)
      if(toBlocks.length) await supabase.from('workout_blocks').insert(toBlocks)
      return true
    }catch{ notify('error','Could not generate this week'); return false }
  }
  ...
  async function loadAll(userId: string){
    ...
    const pdIds = pds.map(p=>p.id); const wdIds = wds.map(w=>w.id)
    ...
    setWeekMeals(byDateMeals); setWeekBlocks(byDateBlocks)
    const today = ymdLocal(new Date())
    setTodayMeals(byDateMeals[today] || []); setTodayBlocks(byDateBlocks[today] || [])
    return { byDateMeals, byDateBlocks }
  }

  // ---- actions ----
  async function openIngredients(meal: Meal){
    const name = meal.recipe_name || ''
    setIngredientsFor(name); setIngredients([])
    const { data } = await supabase.from('recipes').select('*').ilike('name', name).maybeSingle()
  ...
  async function addIngredientsToGrocery(items: string[]){
    ...
    for(const raw of items){ const n=normalizeName(raw); counts[n] = (counts[n]||0) + 1 }
  ...
  // --- small UI helpers ---
  const segBtn   = (label:string, active:boolean, onClick:()=>void) => <button className={`segbtn ${active?'on':''}`} onClick={onClick}>{label}</button>
