...
export default function Plans(){
  ...
  async function loadAll(userId: string){
    try{
      ...
      // TODAY diet
      const dayQ = await supabase.from('plan_days').select('id').eq('user_id', userId).eq('date', todayStr).maybeSingle()
      if (dayQ.error) throw dayQ.error
      if (dayQ.data){
        const msQ=await supabase.from('plan_meals').select('id,plan_day_id,meal_type,recipe_name').eq('plan_day_id', dayQ.data.id).order('meal_type')
        if (msQ.error) throw msQ.error
        setTodayMeals((msQ.data as any[])||[])
      } else setTodayMeals([])

      // TODAY workout
      const wdayQ=await supabase.from('workout_days').select('id').eq('user_id', userId).eq('date', todayStr).maybeSingle()
      if (wdayQ.error) throw wdayQ.error
      if (wdayQ.data){
        const blocksQ=await supabase.from('workout_blocks').select('id,workout_day_id,type,movements').eq('workout_day_id', wdayQ.data.id)
        if (blocksQ.error) throw blocksQ.error
        setTodayBlocks((blocksQ.data as any[])||[])
      } else setTodayBlocks([])

      // WEEK diet
      const mon=mondayOfWeekContaining(new Date()); const weekDates=datesMonToSun(mon).map(ymdLocal)
      const daysQ=await supabase.from('plan_days').select('id,date').eq('user_id', userId).in('date', weekDates)
      ...
      if(ids.length){
        const allMealsQ=await supabase.from('plan_meals').select('id,plan_day_id,meal_type,recipe_name').in('plan_day_id',ids)
        if(allMealsQ.error) throw allMealsQ.error
        const grouped:Record<string,Meal[]> = {}
        for (const m of (allMealsQ.data as any[])||[]){
      ...
      // WEEK workout
      const wdaysQ=await supabase.from('workout_days').select('id,date').eq('user_id', userId).in('date', weekDates)
      if (wdaysQ.error) throw wdaysQ.error
      const wMap:Record<string,string>={}; for(const d of (wdaysQ.data||[])) wMap[(d as any).date]=(d as any).id
      const wIds=Object.values(wMap)
      if(wIds.length){
        const allBlocksQ=await supabase.from('workout_blocks').select('id,workout_day_id,type,movements').in('workout_day_id',wIds)
        if(allBlocksQ.error) throw allBlocksQ.error
        const grouped:Record<string,WorkoutBlock[]> = {}
        for (const b of (allBlocksQ.data as any[])||[]){
  ...
  async function onGenerateDietToday(){
    ...
    try{ const { data: { session } } = await supabase.auth.getSession(); const userId=session?.user?.id; if(!userId){ notify('error','Please sign in again'); return }
      await ensureDietForDate(userId, todayStr); notify('success','Diet generated for today'); await loadAll(userId)
  ...
  async function onGenerateDietWeek(){
    setBusyDiet(true); setLastError('')
    try{ const { data: { session } } = await supabase.auth.getSession(); const userId=session?.user?.id; if(!userId){ notify('error','Please sign in again'); return }
      const mon=mondayOfWeekContaining(new Date()); for(const d of datesMonToSun(mon)) await ensureDietForDate(userId, ymdLocal(d)); notify('success','Diet generated for this week'); await loadAll(userId)
  ...
  async function onGenerateWorkoutToday(){
    ...
    try{ const { data: { session } } = await supabase.auth.getSession(); const userId=session?.user?.id; if(!userId){ notify('error','Please sign in again'); return }
      await ensureWorkoutForDate(userId, todayStr); notify('success','Workout generated for today'); await loadAll(userId)
  ...
  async function onGenerateWorkoutWeek(){
    setBusyWorkout(true); setLastError('')
    try{ const { data: { session } } = await supabase.auth.getSession(); const userId=session?.user?.id; if(!userId){ notify('error','Please sign in again'); return }
      const mon=mondayOfWeekContaining(new Date()); for(const d of datesMonToSun(mon)) await ensureWorkoutForDate(userId, ymdLocal(d)); notify('success','Workout generated for this week'); await loadAll(userId)
  ...
  async function addIngredient(name:string){
    const { data: { session } } = await supabase.auth.getSession(); const userId=session?.user?.id; if(!userId){ notify('error','Please sign in again'); return }
    const prof=await supabase.from('profiles').select('family_id').eq('id',userId).maybeSingle(); if(prof.error){ setLastError(prof.error.message); return }
    ...
    <div className="grid">
      ...
      {!hasUser ? (
        <div className="card">
          ...
          <p className="muted">Your session isn’t available. Try signing in again from the header.</p>
        </div>
      ) : (
        <>
          <div className="card">
          ...
          <div className="card">
            ...
            {workoutView==='today' ? (
              ...
              <WeekBlocks weekBlocks={weekBlocks} />
            )}
          </div>
        </>
      )}
...
function WeekMeals({ weekMeals, ...rest }:{ weekMeals: DayMeals[] } & any){
  return (<div className="grid">{weekMeals.map(d=>(
    <div key={d.date} className="card">
      <b>{d.date}</b>
      <MealsForToday todayMeals={d.meals} {...rest} />
    </div>
  ))}</div>)
}
