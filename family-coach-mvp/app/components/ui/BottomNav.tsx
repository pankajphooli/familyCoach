...
function Icon({name, active=false}:{name:IconName; active?:boolean}){
  ...
  switch(name){
    ...
    case 'grocery':
      return (
        <svg {...svgProps}>
          <path d="M6 6h15l-1.5 8.5a2 2 0 0 1-2 1.6H9.5a2 2 0 0 1-2-1.6L5 3H3" />
        ...
        {items.map(it=>{
          ...
          return (
            <li key={it.href} className="flex flex-col items-center justify-center">
              <a href={it.href} className="flex flex-col items-center justify-center" aria-label={it.label}>
