...
type Member = { user_id: string; role: 'owner'|'member'; can_manage_members?: boolean|null; profiles?: Profile|null }
...
export default function FamilyPage(){
  ...
  useEffect(()=>{ (async()=>{
    ...
    // Find my family id (from profiles or membership)
    let famId: string|undefined
    const p = await supabase.from('profiles').select('family_id').eq('id', user.id).maybeSingle()
  ...
  async function onRemoveAdult(uid:string){
    if(!fam) return
    if(!canManage){ notify('error','You do not have permission'); return }
    if(uid===meId && members.some(m=>m.role==='owner' && m.user_id===uid)){
      notify('error','Owner cannot remove themselves'); return
    }
    const { error } = await supabase.from('family_members').delete().eq('family_id', fam.id).eq('user_id', uid)
    if(error){ notify('error','Remove failed'); return }
  ...
  async function onAddKid(){
    ...
    const { data, error } = await supabase.from('dependents')
      .insert({ family_id: fam.id, name, dob: kidDob || null })
      .select('id,name,dob').maybeSingle()
    if(error){ notify('error','Add failed'); return }
    setKids([...(kids||[]), data as Dependent])
    setKidName(''); setKidDob('')
    notify('success','Child added')
  }

  function roleBadges(m:Member){
    const tags:string[] = []
  ...
  return (
    <div className="container">
      ...
      <div className="fam-row">
        <div className="fam-title">Family: <span className="fam-name">{fam.name || '—'}</span></div>
        <button className="btn btn-dark" onClick={()=>addCardRef.current?.scrollIntoView({behavior:'smooth'})}>Add Kids</button>
      </div>
      ...
      <div className="invite-row">
        ...
        <button className="btn btn-dark" onClick={onRegenerate}>Regenerate</button>
      </div>
            ...
            <li key={m.user_id} className="member-row">
              <div className="member-left">
                ...
                <span className="m-tags">
                  {roleBadges(m).map(t=> <span key={t} className="tag">({t})</span>)}
                </span>
              </div>
              <div className="member-right">
                {canManage && <button className="btn btn-outline sm" onClick={()=>onRemoveAdult(m.user_id)}>Remove</button>}
              </div>
            </li>
          ))}
          {kids.map((k, idx)=>(
            <li key={k.id} className="member-row">
              <div className="member-left">
                ...
                <span className="m-tags"><span className="tag">(child)</span>{k.dob ? <span className="dob">  DOB {new Date(k.dob).toLocaleDateString()}</span> : null}</span>
              </div>
              <div className="member-right">
                {canManage && <button className="btn btn-outline sm" onClick={()=>onRemoveKid(k.id)}>Remove</button>}
              </div>
            </li>
          ))}
